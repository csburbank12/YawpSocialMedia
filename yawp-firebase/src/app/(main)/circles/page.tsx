...
export default function CirclesPage() {
  ...
  const sendMsg = async () => {
    if (!msg.trim() || !selected || !user) return
  ...
  if (selected) return (
    <div style={{ maxWidth:640, margin:'0 auto', padding:'24px 16px' }}>
      ...
      <div style={{ background:'#141414', border:'1px solid #2A2A2A', borderRadius:16, padding:'16px 20px', marginBottom:20 }}>
      ...
      <div style={{ flex:1, overflowY:'auto', maxHeight:400, display:'flex', flexDirection:'column', gap:14, marginBottom:16 }}>
        {messages.map(m => (
          <div key={m.id} style={{ display:'flex', gap:10, flexDirection: m.userId === user?.uid ? 'row-reverse' : 'row' }}>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key==='Enter' && sendMsg()} placeholder="Say something..."
        ...
        <button onClick={sendMsg} style={{ background:selected.color, border:'none', borderRadius:20, padding:'10px 18px', color:'#0D0D0D', fontWeight:700, cursor:'pointer' }}>Send</button>
      </div>
    </div>
  )
    ...
    <div style={{ maxWidth:640, margin:'0 auto', padding:'24px 16px' }}>
      ...
      {circles.map(c => (
        <div key={c.id} onClick={() => setSelected(c)}
          style={{ background:'#141414', border:'1px solid #2A2A2A', borderRadius:14, padding:'16px 18px', marginBottom:10, cursor:'pointer', display:'flex', alignItems:'center', gap:14, transition:'border-color 0.2s' }}
      ...
      {creating ? (
        <div style={{ background:'#141414', border:'1px solid #3A3A3A', borderRadius:14, padding:'16px 18px' }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={createCircle} style={{ flex:1, background:'#E8FF47', border:'none', borderRadius:20, padding:9, color:'#0D0D0D', fontWeight:700, cursor:'pointer' }}>Create</button>
          </div>
        </div>
      ) : (
