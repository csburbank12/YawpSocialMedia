...
function renderContentWithHashtags(content: string, router: ReturnType<typeof useRouter>) {
  const parts = content.split(/(#\w+)/g)
  return parts.map((part, i) => {
    if (part.startsWith('#')) {
      return (
        <span key={i} onClick={e => { e.stopPropagation(); router.push(`/tag/${encodeURIComponent(part.replace('#', ''))}`) }}
...
export default function PostPage({ params }: { params: { id: string } }) {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      toast('Failed to reply. Try again.', 'error')
    }
    setPosting(false)
  }
  ...
  if (!post) return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ background: '#141414', borderRadius: 16, padding: '18px 20px', marginBottom: 16, animation: 'pulse 1.5s infinite' }}>
        <div style={{ height: 16, borderRadius: 8, marginBottom: 12, width: '60%' }} className="skeleton" />
        <div style={{ height: 14, borderRadius: 8, marginBottom: 8, width: '90%' }} className="skeleton" />
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={sharePost} style={{ background: 'none', border: '1px solid #2A2A2A', borderRadius: 20, color: '#555', cursor: 'pointer', fontSize: 12, padding: '4px 12px', transition: 'all 0.15s' }}
      ...
      {/* Original post */}
      <div className="animate-fade-in" style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, padding: '18px 20px', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ color: '#F0F0F0', fontSize: 15, lineHeight: 1.6, margin: '0 0 12px', fontFamily: 'Georgia,serif' }}>
              {renderContentWithHashtags(post.content, router)}
            </p>
            {post.tags?.length > 0 && (
              <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
                {post.tags.map(tag => <span key={tag} onClick={() => router.push(`/tag/${encodeURIComponent(tag.replace('#', ''))}`)} style={{ background: '#1E1E1E', color: '#E8FF47', fontSize: 11, padding: '2px 8px', borderRadius: 20, fontFamily: "'DM Mono',monospace", cursor: 'pointer' }}>{tag}</span>)}
              </div>
            )}
      ...
      <div style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, overflow: 'hidden' }}>
        {replies.length === 0 ? (
          ...
          return (
            <div key={reply.id} className="animate-fade-in-up" style={{ display: 'flex', gap: 12, padding: '14px 20px', borderBottom: i < replies.length - 1 ? '1px solid #1E1E1E' : 'none', animationDelay: `${i * 0.04}s` }}>
