...
import { collection, query, where, orderBy, onSnapshot, addDoc, doc, getDoc, getDocs, setDoc } from 'firebase/firestore'
...
export default function MessagesPage() {
  ...
  const startConversation = async (otherUser: Profile) => {
    ...
    try {
      const ref = await addDoc(collection(db, 'conversations'), {
        ...
        lastMessage: '', lastMessageAt: Date.now(), lastSenderId: user.uid, createdAt: Date.now(),
      })
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'conversations', selected.conv.id), {
        lastMessage: text, lastMessageAt: Date.now(), lastSenderId: user.uid,
      }, { merge: true })
    } catch {
      setInput(text)
  ...
  if (selected) return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 130px)' }}>
      <div className="animate-fade-in" style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 16, borderBottom: '1px solid #2A2A2A', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, padding: '16px 0' }}>
        {messages.length === 0 && (
          <div className="animate-fade-in" style={{ textAlign: 'center', color: '#555', padding: '40px 20px', fontFamily: 'Georgia,serif', fontSize: 14 }}>
        ...
        {messages.map(m => {
          ...
          return (
            <div key={m.id} className="animate-fade-in" style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8 }}>
              <div>
      ...
      <div style={{ display: 'flex', gap: 8, paddingTop: 12, borderTop: '1px solid #2A2A2A', flexShrink: 0 }}>
        ...
        <button onClick={sendMessage} disabled={!input.trim() || sending} style={{ background: input.trim() ? '#E8FF47' : '#2A2A2A', border: 'none', borderRadius: 20, padding: '10px 18px', color: input.trim() ? '#0D0D0D' : '#555', fontWeight: 700, fontSize: 13, cursor: input.trim() ? 'pointer' : 'default', flexShrink: 0, transition: 'all 0.15s' }}>↑</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
      ...
      {composing && (
        <div className="animate-fade-in-up" style={{ background: '#141414', border: '1px solid #3A3A3A', borderRadius: 14, padding: 16, marginBottom: 16 }}>
      ...
      {conversations.length === 0 && !composing ? (
        ...
        return (
          <div key={conv.id} className="animate-fade-in-up" onClick={() => setSelected({ conv, otherUser: other })}
            style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 14, padding: '14px 16px', marginBottom: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14, transition: 'border-color 0.2s', animationDelay: `${i * 0.05}s` }}
