...
export default function ProfilePage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px' }}>
      {/* Profile card */}
      <div className="animate-fade-in" style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, padding: 24, marginBottom: 16 }}>
      ...
      {/* Yawp+ */}
      {!profile.isPlus && (
        <div className="animate-fade-in-up" style={{ background: 'linear-gradient(135deg, #141414 0%, #1A1500 100%)', border: '1px solid #2A2200', borderRadius: 16, padding: '20px 24px', marginBottom: 24 }}>
          ...
          <button onClick={() => toast('Yawp+ is coming soon!', 'info')} style={{ background: '#E8FF47', border: 'none', borderRadius: 20, padding: '9px 20px', color: '#0D0D0D', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            Join Yawp+ — $5/mo
          </button>
        </div>
      )}
      ...
      {loading ? (
        <div>
          {[1, 2].map(i => (
            <div key={i} style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 14, padding: '14px 18px', marginBottom: 10 }}>
              <div style={{ height: 14, width: '80%', marginBottom: 8 }} className="skeleton" />
              <div style={{ height: 14, width: '50%', marginBottom: 12 }} className="skeleton" />
              <div style={{ height: 12, width: 120 }} className="skeleton" />
            </div>
          ))}
        </div>
      ) : posts.length === 0 ? (
        <div className="animate-fade-in" style={{ textAlign: 'center', color: '#555', padding: '40px 20px' }}>
        ...
        <div key={post.id} className="animate-fade-in-up" onClick={() => router.push(`/post/${post.id}`)}
          style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 14, padding: '14px 18px', marginBottom: 10, position: 'relative', cursor: 'pointer', transition: 'border-color 0.2s', animationDelay: `${i * 0.04}s` }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = '#3A3A3A'; const btn = e.currentTarget.querySelector<HTMLButtonElement>('.del-btn'); if (btn) btn.style.opacity = '1' }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = '#2A2A2A'; const btn = e.currentTarget.querySelector<HTMLButtonElement>('.del-btn'); if (btn) btn.style.opacity = '0' }}>
